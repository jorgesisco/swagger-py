...
class Operation(object):
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}({self.json['nickname']})"
    ...
    def __call__(self, **kwargs):
        ...
        """
        log.info(f"{self.json['nickname']}?{urlencode(kwargs)}")
        ...
        for param in self.json.get('parameters', []):
            ...
            if value:
                if param['paramType'] == 'path':
                    uri = uri.replace(f'{{{pname}}}', str(value))
                ...
                if param['required']:
                    raise TypeError(
                        f"Missing required parameter '{pname}' for '{self.json['nickname']}'")
        if kwargs:
            raise TypeError(f"'{self.json['nickname']}' does not have parameters {kwargs.keys()}")

        log.info(f"{method} {uri}({params!r})")
...
class Resource(object):
    ...
    def __init__(self, resource, http_client):
        log.debug(f"Building resource '{resource['name']}'")
        self.json = resource
    ...
    def __getattr__(self, item):
        ...
        if not op:
            raise AttributeError(f"Resource '{self.get_name()}' has no operation '{item}'")
        return op
    ...
    def _build_operation(self, decl, api, operation):
        ...
        """
        log.debug(f"Building operation {self.get_name()}.{operation['nickname']}")
...
class SwaggerClient(object):
    ...
    def __init__(self, url_or_resource, http_client=None):
        ...
        if isinstance(url_or_resource, str):
            log.debug(f"Loading from {url_or_resource}")
            self.api_docs = loader.load_resource_listing(url_or_resource)
        else:
            log.debug(f"Loading from {url_or_resource.get('basePath')}")
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}({self.api_docs['basePath']})"
    ...
    def __getattr__(self, item):
        ...
        if not resource:
            raise AttributeError(f"API has no resource '{item}'")
        return resource
