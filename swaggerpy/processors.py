...
class ParsingContext(object):
    ...
    def __repr__(self):
        ...
        strs = [f"{t}={i}" for (t, i) in zipped]
        return f"ParsingContext(stack={strs})"
    ...
    def push(self, obj_type, json, id_field):
        ...
        if id_field not in json:
            raise SwaggerError(f"Missing id_field: {id_field}", self)
...
class SwaggerProcessor(object):
    ...
    def apply(self, resources):
        ...
        for listing_api in resources['apis']:
            ...
            context.pop()
        context.pop()
        assert context.is_empty(), f"Expected {context} to be empty"
